...
'''
This program takes P, an Mx2 numpy array of M boundary points with x, y cartesian coordinates, 
and generates A, an Nx2 list of all N coordinates that fall within the area that is enclosed by sequentially connecting the boundary points of P

Assumes coordinates of P are given as clockwise-sequential integer pairs.

An example case is provided and solved with test points which are output as boolean values corresponding to 'inside' or 'outside'.

This program uses a point-in-polygon ray tracing method.  See supporting pdf for theory.
'''
